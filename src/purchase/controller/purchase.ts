...
import { addPurchase, afterClickingTheSubmitButton, purchaseList ,getPurchaseDetailsById, editPurchaseById} from "../service/purchase";
...
router.patch("/purchase", async (req: Request, res: Response) => {
  editPurchaseById(req.query.id,req.body)
    .then((data) => res.send(data))
    .catch((data) => res.send(data));
});
